"""Toplama isi. Haftada 2-3 calisir. Tara, sec, yaz, gorsel, Notion'a taslak."""
import datetime
import sys
import feedparser
...
REQUIRED = ["ANTHROPIC_API_KEY", "UNSPLASH_ACCESS_KEY", "NOTION_TOKEN", "NOTION_DB_ID"]
WINDOW_DAYS = 7
MAX_PER_FEED = 5
...
def main():
    ...
    for pick in chosen:
        ...
        img = images.search(written["image_query"])

        kaynaklar = f"{item['source']} :: {item['link']}"
        gorsel = f"{img['url']} :: {img['credit']} / Unsplash :: {img['link']}"
