"""Notion taslak kuyrugu I/O. Bu sohbetin disinda, kendi token'iyla calisir."""
import os
import requests
...
def create_draft(baslik, kategori, kiyi, kaynaklar, gorsel, yayin_tarihi, body):
    ...
    payload = {
        ...
        "properties": {
            ...
            "Kıyı": {"select": {"name": kiyi}},
            "Durum": {"select": {"name": "Taslak"}},
            "Kaynaklar": {"rich_text": [{"text": {"content": kaynaklar[:1900]}}]},
            "Görsel Bağlantısı": {"rich_text": [{"text": {"content": gorsel[:1900]}}]},
